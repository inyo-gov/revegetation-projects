...
datatable(
  ...
  options = list(
    ...
    scrollX = FALSE,
                    columnDefs = list(
                      list(width = '8%', targets = 0),     # parcel
                      list(width = '3%', targets = 1),     # year
                      list(width = '9%', targets = 2),     # full cover
                      list(width = '9%', targets = 3),     # capped cover
                      list(width = '16%', targets = 4),    # original cover (Excl. ATTO/ERNA/ATPO)
                      list(width = '11%', targets = 5),    # species ≥3 hits
                      list(width = '11%', targets = 6),    # species richness
                      list(width = '11%', targets = 7),    # transect cover
                      list(width = '11%', targets = 8),    # grass species
                      list(width = '6%', targets = 9)      # compliance summary
                    ),
    dom = 'Bfrtip',
    buttons = c('csv'),
    pageLength = 20
...
This table shows the average cover by species for revegetation parcels (LAW090, LAW094, LAW095, and LAW129_118), grouped by reference group. It helps assess which species are most dominant in the revegetation areas and provides context for understanding the success of revegetation efforts compared to reference conditions.
...
revegetation_species_cover <- r3_data %>%
  filter(parcel %in% c('LAW090', 'LAW094', 'LAW095', 'LAW129_118')) %>%
  mutate(
  ...
  group_by(reference_group, species, Lifecycle) %>%
  summarise(
    avg_cover = mean(transect_cover, na.rm = TRUE),
    n_transects = n(),
    .groups = 'drop'
  ) %>%
  filter(avg_cover > 0) %>%  # Only show species with some cover
  arrange(reference_group, desc(avg_cover)) %>%
  mutate(
...
```{r, warning=FALSE}
#| tbl-cap: "Average cover by species for revegetation parcels. Note: LAW118/129 data represents the combined LAW129_118 parcel only."
#| column: screen
