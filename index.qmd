---
title: "Laws Type E Transfer Revegetation"
subtitle: "LAW090, LAW094, LAW095, LAW118/129 — abandoned Type E agriculture to precip-dependent native community (2003 plan)"
...
**What:** Working compliance report for Laws **Type E transfer** parcels under the 2003 revegetation plan [@LADWP2003; @LADWP2003MND].

**Why:** Annual cover and composition scores are the decision artifacts partners need; publishing them with methods makes the path from hits → goals auditable. Pages marked *(draft)* elsewhere are prototypes—see [goals & caps summary](amendment_summary.html) for shared direction vs the open ATTO/ERNA cap question.
:::

## Purpose

**What is Type E transfer?** Land retired from irrigated agriculture (Green Book Type E) and managed toward a **precipitation-dependent** native community—not kept in farming [@InLA1991; @LADWP2003MND]. This report tracks **LAW090, LAW094, LAW095**, and **LAW118/129**.

**Why these metrics?** Dryland succession can take decades; plan goals are milestones along that path, not proofs of climax. Absolute cover and richness answer *compliance*; [STM](stm.html) and [NMDS](nmds.html) address *ecological state and composition*. Companion [reference parcels](reference.html) supply empirical bases for ATTO/ERNA caps.^[Caps limit how much of each nurse/shrub species may count toward the 10% cover goal so one species cannot dominate the score.] Other 1991 EIR / 1999 Plan sites are reported separately ([1999 Plan monitoring](reveg1999plan.html)).

**Default vs scenarios.** Tables below use the **working** cap stack. [Cap scenarios](#cap-scenarios) re-score the same hits under alternate cut points without replacing the primary analysis.

### Revegetation targets (working rules)

All five must pass for overall attainment (**✓** attained / **✗** not):

1. **Perennial cover ≥ 10%** — parcel mean of transect cover from allowable species (after caps).^[Cover = (hits / 200) × 100 per transect (fixed 200 intercepts), then mean across transects.]
2. **≥6 species with ≥3 hits** — from allowable list plus ATTO, ERNA10, ATPO.
3. **Species richness ≥ 10** — distinct perennial species from that same list.
4. **Every transect ≥ 2%** cover (same species filter).
5. **≥1 grass** species from the allowable list.

**Working caps:** LAW090/094/095 — ATTO and ERNA10 at **0.3%** each (ATPO uncapped on LAW090). LAW118/129 — ATTO/ERNA10 **2%** each, ATPO **3%**. Alternate stacks live in `data/cap_scenarios.csv`.

## Location

Parcel boundaries and monitoring transect locations are shown in @fig-laws-map. Click transects for details; use the layer control to toggle features.

```{r}
#| label: fig-laws-map
#| fig-cap: "Laws Type E revegetation parcels and monitoring transect start locations."
#| echo: false
#| warning: false
#| message: false
...
write_sf(parcels_sf, "docs/laws_parcels.geojson")
write_sf(all_transects_combined, "output/laws_transects.geojson")
write_sf(all_transects_combined, "docs/laws_transects.geojson")
write.csv(st_drop_geometry(all_transects_combined), "output/laws_transects.csv", row.names = FALSE)
write.csv(st_drop_geometry(all_transects_combined), "docs/laws_transects.csv", row.names = FALSE)
...
**Capping Rules by Parcel Group (working default)** - **LAW90/94/95**: ERNA10 and ATTO capped at 0.3% each, ATPO unlimited for LAW090 - **LAW118/129**: ERNA10/ATTO capped at 2% each, ATPO capped at 3%. Scenario variants: `data/cap_scenarios.csv`.
...
# Cap scenarios (working default + LADWP draft + lower/higher sensitivity)
cap_scenarios <- read_csv("data/cap_scenarios.csv", show_col_types = FALSE)
DEFAULT_CAP_SCENARIO <- "working"

parcel_group_for <- function(parcel) {
  if (parcel %in% c("LAW090", "LAW094", "LAW095")) {
    "LAW90_94_95"
  } else if (parcel %in% c("LAW118/129", "LAW118", "LAW129")) {
    "LAW118_129"
  } else {
    NA_character_
  }
...
lookup_cap <- function(scenario, parcel, species) {
  grp <- parcel_group_for(parcel)
  hit <- cap_scenarios %>%
    filter(
      .data$scenario == !!scenario,
      .data$parcel_group == !!grp,
      .data$species == !!species
    )
  if (nrow(hit) == 0) {
    return(NA_real_) # no row → uncapped
  }
  as.numeric(hit$cap_pct[[1]])
}

# Function to calculate cover and capping for a parcel/year
calculate_parcel_cover <- function(parcel, year, scenario = DEFAULT_CAP_SCENARIO) {
  ...
  if (parcel == "LAW118/129" && year == 2025 && scenario == DEFAULT_CAP_SCENARIO) {
  ...
  # Apply capping rules from cap_scenarios for this scenario
  if(nrow(parcel_allowable) > 0) {
    parcel_species_cover <- parcel_allowable %>%
      ...
      mutate(
        ...
        cap_pct = vapply(species, function(sp) lookup_cap(scenario, parcel, sp), numeric(1)),
        capped_cover = ifelse(is.na(cap_pct), species_cover, pmin(species_cover, cap_pct))
      )
...
# Visual audit: Show analytical results data (working caps)
cat("\n=== Analytical Results Data (scenario =", DEFAULT_CAP_SCENARIO, ") ===\n")
...
# Display wider table
if(nrow(dashboard_display) > 0) {
DT::datatable(
  dashboard_display,
    caption = 'Analytical Results Table (working caps)',
    options = list(
...
**What:** Recompute the cover ≥10% test under alternate ATTO / ERNA10 / ATPO cut points (`working`, `ladwp_draft`, `lower`, `higher`).

**Why:** Cap choice changes how much of the hit data can count toward the goal. Parameterizing stacks in [`data/cap_scenarios.csv`](data/cap_scenarios.csv) lets us simulate perturbations without rewriting narrative each time—see [goals & caps summary](amendment_summary.html). Primary tables above stay on **`working`**.

| Scenario | LAW090/094/095 ATTO / ERNA | LAW118/129 ATTO / ERNA / ATPO |
|----------|----------------------------|--------------------------------|
| **working** | 0.3 / 0.3 | 2.0 / 2.0 / 3.0 |
| **ladwp_draft** | 0.1 / 0.3 | 2.8 / 4.3 / 3.0 |
| **lower** | 0.1 / 0.1 | 1.5 / 1.5 / 3.0 |
| **higher** | 0.5 / 0.5 | 4.0 / 5.0 / 3.0 |

```{r cap-scenario-comparison}
#| echo: false
#| warning: false
#| message: false

scenario_ids <- unique(cap_scenarios$scenario)
scenario_labels <- cap_scenarios %>%
  distinct(scenario, label)
scenario_label_map <- setNames(scenario_labels$label, scenario_labels$scenario)

# Latest year per parcel from working dashboard
parcel_years <- dashboard_data %>%
  group_by(Parcel) %>%
  summarise(Year = max(Year), .groups = "drop")

cap_compare <- purrr::map_dfr(scenario_ids, function(sc) {
  purrr::pmap_dfr(parcel_years, function(Parcel, Year) {
    calculate_parcel_cover(Parcel, Year, scenario = sc)
  })
}) %>%
  mutate(
    label = unname(scenario_label_map[Scenario]),
    cover_capped = `Cover After Species Caps`,
    cover_ok = cover_capped >= 10
  )

cap_wide <- cap_compare %>%
  select(Parcel, Year, Scenario, cover_capped, cover_ok) %>%
  tidyr::pivot_wider(
    names_from = Scenario,
    values_from = c(cover_capped, cover_ok),
    names_glue = "{Scenario}_{.value}"
  )

# Readable long table for DT
cap_compare_dt <- cap_compare %>%
  transmute(
    Parcel,
    Year,
    Scenario = label,
    `Uncapped cover %` = `Cover %`,
    `Capped cover %` = cover_capped,
    `≥10%?` = ifelse(cover_ok, "✓", "✗")
  ) %>%
  arrange(Parcel, Year, Scenario)

DT::datatable(
  cap_compare_dt,
  caption = "Cover after ATTO/ERNA/ATPO caps by scenario (latest year per parcel)",
...
pass_matrix <- cap_compare %>%
  mutate(cell = paste0(round(cover_capped, 1), ifelse(cover_ok, " ✓", " ✗"))) %>%
  select(Parcel, Year, Scenario, cell) %>%
  tidyr::pivot_wider(names_from = Scenario, values_from = cell)

knitr::kable(
  pass_matrix,
  caption = "Capped cover % and ≥10% pass (✓/✗) by scenario"
)
```
...
# Get the analytical results (cover targets) — prefer names over positions (Scenario column added)
if (exists("dashboard_data") && "Cover Status" %in% names(dashboard_data)) {
  cover_results <- dashboard_data %>%
    select(Parcel, Year, `Cover Status`) %>%
    rename(parcel = Parcel, year = Year, cover_status = `Cover Status`)
  
  # Debug: Check cover_results
  cat("Cover results dimensions:", dim(cover_results), "\n")
  print(head(cover_results, 2))
} else {
  cat("ERROR: dashboard_data doesn't have Cover Status or doesn't exist\n")
...
**LAW090/094/095 Parcels (2022-2025)** - **Source**: Los Angeles Department of Water and Power (LADWP) - **Files**: - [2022 Data](laws_reveg_data_2022.csv) - LAW090/094/095 monitoring data - [2024 Data](laws_reveg_data_2024.csv) - LAW090/094/095 monitoring data - [2025 Data](LADWP_ReferenceParcel_LAW090_094_095_2025_Data_2025.xlsx) - LAW090/094/095 Excel data - **Content**: Point-intercept survey data with species hits per transect - **Cover Conversion**: Raw hits converted to percentage cover (hits/200 × 100) - **Years**: 2022, 2024, 2025 (no monitoring in 2023)

**LAW118/129 Parcels (2022-2025)** - **Source**: Inyo County Water Department (ICWD) - **Files**: - [2022 Data](laws_reveg_data_2022.csv) - LAW118/129 monitoring data - [2023 Data](laws118_reveg_data_2023.csv) - LAW118/129 monitoring data - [2025 Data](LAW118_129_reveg2025_e.csv) - LAW118/129 monitoring data - **Content**: Point-intercept survey data for combined LAW118/129 parcel - **Cover Conversion**: Raw hits converted to percentage cover (hits/200 × 100) - **Years**: 2022, 2023, 2025 (no monitoring in 2024)

### 🔧 Auxiliary Data

#### Species Reference Data

-   **File**: [TypeE_Transfer_SppList.xlsx](TypeE_Transfer_SppList.xlsx)
...
-   **File**: [species.csv](species.csv) - Species codes, names, and classifications
...
-   [**Reference Parcel Summary**](reference_parcel_summary.csv) - Reference parcel statistics and ATTO/ERNA analysis
...
# Copy spatial files to docs/ if they exist in output/
if (file.exists("output/laws_parcels.geojson")) {
  file.copy("output/laws_parcels.geojson", "docs/laws_parcels.geojson", overwrite = TRUE)
}
if (file.exists("output/laws_transects.geojson")) {
  file.copy("output/laws_transects.geojson", "docs/laws_transects.geojson", overwrite = TRUE)
}
if (file.exists("output/laws_transects.csv")) {
  file.copy("output/laws_transects.csv", "docs/laws_transects.csv", overwrite = TRUE)
}

# Copy reference parcel files to docs/ if they exist in output/
if (file.exists("output/reference_parcel_summary.csv")) {
  file.copy("output/reference_parcel_summary.csv", "docs/reference_parcel_summary.csv", overwrite = TRUE)
}
if (file.exists("output/reference_atto_erna_analysis.csv")) {
  file.copy("output/reference_atto_erna_analysis.csv", "docs/reference_atto_erna_analysis.csv", overwrite = TRUE)
}

# Copy primary monitoring data files to docs/
if (file.exists("data/raw/reveg/laws_reveg_data_2022.csv")) {
  file.copy("data/raw/reveg/laws_reveg_data_2022.csv", "docs/laws_reveg_data_2022.csv", overwrite = TRUE)
}
if (file.exists("data/raw/reveg/laws_reveg_data_2024.csv")) {
  file.copy("data/raw/reveg/laws_reveg_data_2024.csv", "docs/laws_reveg_data_2024.csv", overwrite = TRUE)
}
if (file.exists("data/raw/reveg/LADWP_ReferenceParcel_LAW090_094_095_2025_Data_2025.xlsx")) {
  file.copy("data/raw/reveg/LADWP_ReferenceParcel_LAW090_094_095_2025_Data_2025.xlsx", 
            "docs/LADWP_ReferenceParcel_LAW090_094_095_2025_Data_2025.xlsx", overwrite = TRUE)
}
if (file.exists("data/raw/reveg/laws118_reveg_data_2023.csv")) {
  file.copy("data/raw/reveg/laws118_reveg_data_2023.csv", "docs/laws118_reveg_data_2023.csv", overwrite = TRUE)
}
if (file.exists("data/raw/reveg/LAW118_129_reveg2025_e.csv")) {
  file.copy("data/raw/reveg/LAW118_129_reveg2025_e.csv", "docs/LAW118_129_reveg2025_e.csv", overwrite = TRUE)
}

# Copy auxiliary data files to docs/
if (file.exists("data/TypeE_Transfer_SppList.xlsx")) {
  file.copy("data/TypeE_Transfer_SppList.xlsx", "docs/TypeE_Transfer_SppList.xlsx", overwrite = TRUE)
}
if (file.exists("data/species.csv")) {
  file.copy("data/species.csv", "docs/species.csv", overwrite = TRUE)
}

cat("Results exported to output/ and docs/ directories\n")
cat("Primary and auxiliary data files copied to docs/ directory\n")
```
