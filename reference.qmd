...
# Note: n_transects in reference_atto_erna_analysis is per species, so we need to take the max
# (not sum) since all species in a parcel-year are sampled on the same transects
transects_by_parcel_year <- reference_atto_erna_analysis %>%
  group_by(parcel, year) %>%
  summarise(
    n_transects = max(n_transects, na.rm = TRUE),  # Take max instead of sum
    .groups = 'drop'
  ) %>%
  pivot_wider(
    names_from = year,
...
# Add reference group column
transects_by_parcel_year <- transects_by_parcel_year %>%
  mutate(
    reference_group = case_when(
      parcel %in% c('LAW012', 'LAW024', 'LAW028', 'LAW048', 'LAW049', 'LAW091', 'LAW093', 'LAW117', 'LAW130', 'LAW134') ~ 'LAW90/94/95',
  ...
  filter(reference_group != 'Other') %>%
  select(reference_group, parcel, everything())
```
...
## Transects Summary by Reference Group and Year

```{r, echo=FALSE}
# Calculate transects summary by reference group and year
# First get unique transects per parcel-year, then sum by group
transects_summary_by_group <- reference_atto_erna_analysis %>%
  mutate(
    reference_group = case_when(
      parcel %in% c('LAW012', 'LAW024', 'LAW028', 'LAW048', 'LAW049', 'LAW091', 'LAW093', 'LAW117', 'LAW130', 'LAW134') ~ 'LAW90/94/95',
      parcel %in% c('LAW029', 'LAW039', 'LAW069', 'LAW104', 'LAW119', 'PLC202', 'PLC219', 'PLC227', 'PLC230') ~ 'LAW118/129',
  ...
  summarise(
    parcel_transects = max(n_transects, na.rm = TRUE),  # Max per parcel-year
    .groups = 'drop'
  ) %>%
  group_by(reference_group, year) %>%
  summarise(
    total_transects = sum(parcel_transects, na.rm = TRUE),
    n_parcels = n_distinct(parcel),
    .groups = 'drop'
  ) %>%
  pivot_wider(
    names_from = year,
    values_from = total_transects,
    names_prefix = "Year_",
    values_fill = 0
  ) %>%
...
```{r, warning=FALSE}
#| tbl-cap: "Total transects by reference group and year"
#| column: screen

datatable(
  transects_summary_by_group,
  options = list(
    autoWidth = TRUE,
    scrollX = FALSE,
    dom = 'Bfrtip',
